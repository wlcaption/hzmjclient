...
public class MyMahjongScript : MonoBehaviour {
  ...
  private List<GameObject> buhuaListB; //buhua
  ...
  private IEnumerator UpdataBattery() {
    while (true) {
      ...
      if (battery > 10) dianliangImage.sprite = Resources.Load("Image/youxizhong_icon3", typeof(Sprite)) as Sprite;
  ...
  public void startGame(ClientResponse response) {
    ...
    LeavedRoundNumText.text = (GlobalDataScript.totalTimes - GlobalDataScript.surplusTimes) + "/" + GlobalDataScript.totalTimes;//刷新剩余圈数
    if (!isFirstOpen) {
      btnActionScript = gameObject.GetComponent<ButtonActionScript>();
      initPanel();
      initArrayList();
    ...
    if (sgvo.bixiahu) bixiahuText.SetActive(true);
  ...
  public void onCardChanged() {
    for (int i = 0; i < handerCardList[0].Count; i++) {
      ...
      if (card.getPoint() >= NanjingConfig.HUA_INDEX) return;
    }
  ...
  public void cardChange(GameObject obj)//
  {
    ...
    if (cardPoint >= NanjingConfig.HUA_INDEX) return;
    if (SocketEventHandle.getInstance().isWaiting) return;
    int handCardCount = handerCardList[0].Count - 1;
    // check hua
    for (int i = 0; i < handerCardList[0].Count; i++) {
      if (handerCardList[0][i].GetComponent<bottomScript>().getPoint() >= NanjingConfig.HUA_INDEX) return;
    }
        ...
        if (gangString != null) {
          ...
          for (int j = 0; j < gangtemps.Length; j++) {
            ...
            if (gangpaiObj.type == "an") {
              ...
              if (pengIndex == -1) pengIndex = GlobalDataScript.reEnterRoomData.playerList[i].paiArray[3][gangpaiObj.cardPiont];
              int pengRealIndex = -1;
              if (pengIndex > -1) {
