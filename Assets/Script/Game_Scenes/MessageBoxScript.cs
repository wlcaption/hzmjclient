...
public class MessageBoxScript : MonoBehaviour {
  ...
  void Start () {
    ...
    SoundCtrl.getInstance().playSoundUI();
		...
		CustomSocket.getInstance ().sendMsg (new MessageBoxRequest(1, GlobalDataScript.loginResponseData.account.uuid, index.ToString()));
		if (myMaj != null) {
  ...
  public void sendMessage() {
    ...
    if (myMaj != null) {
      if (string.IsNullOrEmpty(input.text) == false) {
        CustomSocket.getInstance().sendMsg(new MessageBoxRequest(0, GlobalDataScript.loginResponseData.account.uuid, input.text));
        hidePanel();
      }
    }
  }
