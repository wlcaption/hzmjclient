...
public class GameToolScript {
  ...
  private bool checkHu(int[] pai) {
    if (remainHu(pai) == 0) return true;

    for (int i = 0; i < pai.Length; i++) {
      if (pai[i] != 0) {
        if (pai[i] == 4) {
          pai[i] = 0;
          if (checkHu(pai)) return true;
          pai[i] = 4;
        }
  ...
  List<int> getHuCount(int[] pai, int cardIndex = -1) {
    ...
    for (int i = 0; i < paiCheck.Length; i++) {
      if (paiCheck[i] > 0) {
        ...
        if (cardIndex > -1) cloneList[cardIndex] -= 1;
        if (cloneList[i] > 3) continue;
        cloneList[i] += 1;
        jangHu = 0;
        if (checkHu(cloneList)) huList.Add(i);
      }
    }
    return huList;
  }
  ...
  public int CheckHu(int[] cardList, int cardIndex, Dictionary<int, int> pengGangList, int flag) {
    ...
    if (count == 14) {
      ...
      else if (fourCardCount == 1 && twoCardCount == 5) result += Rule.Hu_Type_Haohuaqidui;
      else if (fourCardCount == 2 && twoCardCount == 3) result += Rule.Hu_Type_Chaohaohuaqidui;
      else if (fourCardCount == 3 && twoCardCount == 1) result += Rule.Hu_Type_Chaochaohaohuaqidui;
    }
    ...
    foreach(var i in pengGangList) {
      if (i.Key >= 0 && i.Key <= 8) wanPengGangCount++;
      if (i.Key >= 9 && i.Key <= 17) tiaoPengGangCount++;
      if (i.Key >= 18 && i.Key <= 26) tongPengGangCount++;
      if (i.Key >= 27 && i.Key <= 30) fengPengGangCount++;
    }

    bool yise = false;
    if (wanCount > 0 && (tiaoCount + tiaoPengGangCount + tongCount + tongPengGangCount) == 0) yise = true;
    if (tiaoCount > 0 && (wanCount + wanPengGangCount + tongCount + tongPengGangCount) == 0) yise = true;
    if (tongCount > 0 && (tiaoCount + tiaoPengGangCount + wanCount + wanPengGangCount) == 0) yise = true;

    if (yise && (fengCount + fengPengGangCount) > 0) result += Rule.Hu_Type_Hunyise;
    else if (yise && (fengCount + fengPengGangCount) == 0) result += Rule.Hu_Type_Qingyise;

    if (oneCardCount == 0 && fourCardCount == 0 && threeCardCount == 0 && twoCardCount == 1) result += Rule.Hu_Type_Quanqiududiao;
    else if (oneCardCount == 0 && fourCardCount == 0 && twoCardCount == 1) result += Rule.Hu_Type_DuiDuihu;

    if ((flag & Rule.Hu_Flag_XiaoGang) > 0) result += Rule.Hu_Type_Xiaogangkaihua;
    if ((flag & Rule.Hu_Flag_DaGang) > 0) result += Rule.Hu_Type_Dagangkaihua;
    ...
    if (pengGangList.ContainsKey(cardIndex)) {
      ...
      if (huCount == 1) {
        if (cardIndex < 27) {
          ...
          // yadang
          if (card > 0 && card < 8) {
            if (cardList[cardIndex] <= 2 && cardList[cardIndex - 1] == cardList[cardIndex] && cardList[cardIndex + 1] == cardList[cardIndex]) yadang = true;
          }

          // bianzhi
          if (card == 2) {
            if (cardList[cardIndex] <= 2 && cardList[cardIndex - 2] == cardList[cardIndex] && cardList[cardIndex - 1] == cardList[cardIndex]) bianzhi = true;
          }
          if (card == 6) {
            if (cardList[cardIndex] <= 2 && cardList[cardIndex + 1] == cardList[cardIndex] && cardList[cardIndex + 2] == cardList[cardIndex]) bianzhi = true;
          }
        }
      }

      if (yadang || bianzhi) result += Rule.Hu_Type_Yajue;
    }

    if (result > 0 || (flag >> 8) >= 4) result += Rule.Hu_Type_Xiaohu;
    return result;
  }
  ...
  private bool remainTing(int[] pai) {
    ...
    for (int i = 0; i < pai.Length; i++) {
      if (pai[i] > 0) {
        if (pai[i] == 1) oneCard++;
        else if (pai[i] == 2) twoCard++;
        if (count < 2) card[count] = i;
        count++;
      }
    }
    if (count == 1) return true;
    if (count == 2) {
      if (twoCard > 0) return true;
      if ((card[1] - card[0]) > 2) return false;
      if (card[0] >= 27) return false;
      if (card[1] >= 27) return false;
      if (card[0] <= 8 && card[1] > 8) return false;
      if (card[0] <= 17 && card[1] > 17) return false;
      return true;
    }
    return false;
  }
  ...
  private bool checkTing(int[] pai) {
    ...
    if (remainTing(pai)) return true;

    for (int i = 0; i < pai.Length; i++) {
      if (pai[i] != 0) {
        if (pai[i] == 4) {
          pai[i] = 0;
          if (checkTing(pai)) return true;
          pai[i] = 4;
        }
  ...
  private int CheckTing(int[] cardList, Dictionary<int, int> pengGangList, int flag) {
    ...
    // check qidui
    if (count == 13) {
      if (twoCardCount == 6) result += Rule.Hu_Type_Qidui;
      else if (fourCardCount == 1 && twoCardCount == 4) result += Rule.Hu_Type_Haohuaqidui;
      else if (fourCardCount == 2 && twoCardCount == 2) result += Rule.Hu_Type_Chaohaohuaqidui;
      else if (fourCardCount == 3) result += Rule.Hu_Type_Chaochaohaohuaqidui;
    }

    bool isTing = false;
    if (result > 0) isTing = true;
    if (isTing == false) {
      if (twoFengCount > 2) return 0;
      if (fourFengCount > 0) return 0;
      int[] cloneList = (int[])cardList.Clone();
      isTing = checkTing(cloneList);
    }
    if (isTing == false) return 0;

    if (result == 0 && count == 13) result += Rule.Hu_Type_Menqing;
    ...
    if (tongCount > 0 && (tiaoCount + tiaoPengGangCount + wanCount + wanPengGangCount) == 0) yise = true;
    if (yise) {
      if ((fengCount + fengPengGangCount) > 0) result += Rule.Hu_Type_Hunyise;
      else result += Rule.Hu_Type_Qingyise;
    }

    if (oneCardCount == 1 && fourCardCount == 0 && twoCardCount == 0 && threeCardCount == 0) result += Rule.Hu_Type_Quanqiududiao;
    else if (oneCardCount == 0 && fourCardCount == 0 && twoCardCount == 2) result += Rule.Hu_Type_DuiDuihu;
    else if (oneCardCount == 1 && fourCardCount == 0 && twoCardCount == 0) result += Rule.Hu_Type_DuiDuihu;
  ...
  public int CheckTing(List<GameObject> handList, List<List<GameObject>> pengGangList, int flag) {
    ...
    for (int i = 0; i < handList.Count; i++) {
      ...
      if (card < NanjingConfig.HUA_INDEX) pai[card]++;
    }
