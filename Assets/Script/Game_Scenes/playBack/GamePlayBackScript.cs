...
public class GamePlayBackScript : MonoBehaviour {
  ...
  public int buhuaCountB;
  public int buhuaCountL;
  public int buhuaCountR;
  public int buhuaCountT;
  //======================================
  ...
  private bool buhuaBeginFlag = false;
    ...
    switch (temp.type) {
      ...
      case 14:
        ...
        if (stepNum < aa.behavieList.Count) {
          ...
          if (temp.type >= 11 && temp.type <= 14) stepAction();
        }
        return;
  ...
  private void buhua(int avaIndex, int cardPoint) {
    ...
    switch (avaIndex) {
      case 0:
        buhuaCountB++;
        buhuaTextB.text = buhuaCountB+"";
        break;
      case 1:
        buhuaCountR++;
        buhuaTextR.text = buhuaCountR + "";
        break;
      case 2:
        buhuaCountT++;
        buhuaTextT.text = buhuaCountT + "";
        break;
      case 3:
        buhuaCountL++;
        buhuaTextL.text = buhuaCountL + "";
        break;
    }

    buhuaEffect(cardPoint, avaIndex);
  }

  private void buhuaBegin(int avaIndex, int cardPoint) {
    if (buhuaBeginFlag == false) {
      buhuaBeginFlag = true;
      SoundCtrl.getInstance().playSoundByAction("buhua", playerItems[avaIndex].getSex(), pathDirString[avaIndex]);
    }
    List<GameObject> tempCardObjs = handerCardList[avaIndex];
    GameObject lastCard = tempCardObjs[tempCardObjs.Count - 1];
    int lastPoint = lastCard.GetComponent<TopAndBottomCardScript>().getPoint();
  ...
  private void buhuaBeginPickCard(int avaIndex, int cardPoint) {
  ...
  private void pickCard(int avaIndex,int cardPoint){
	...
	private void qiangGangHu(int avaIndex,int cardPoint){
		...
		SoundCtrl.getInstance ().playSoundByAction ("hu",playerItems[avaIndex].getSex(), pathDirString[avaIndex]);
	...
	{
		SoundCtrl.getInstance ().playSound (cardPoint,playerItems[curAvatarIndex].getSex(), pathDirString[curAvatarIndex]);
    ...
    */
	...
	private void frontAction(){
      ...
      if (temp.type == 1) {
        ...
        frontLiuju();
      } else if (temp.type >= 11 && temp.type <= 14) {
        frontBuhua(tempcurIndex, int.Parse(temp.cardIndex));
        stepNum--;
        SetDirGameObjectAction(tempcurIndex);
        frontAction();
        return;
      } else if (temp.type == 15) {
        frontBuhuaBegin(tempcurIndex, int.Parse(temp.cardIndex));
        stepNum--;
        frontAction();
        return;
      } else if (temp.type == 16) {
        frontBuhuaBeginPickCard(tempcurIndex, int.Parse(temp.cardIndex));
        stepNum--;
        frontAction();
        return;
      }
  ...
  private void frontBuhua(int avaIndex, int cardPoint) {
    GameObject tempObj = null;
  ...
  private void frontBuhuaBegin(int avaIndex, int cardPoint) {
    GameObject tempObj = null;
    tempObj = createGameObjectAndReturn("Prefab/playBack/HandCard_" + pathDirString[avaIndex], parentList[avaIndex], Vector3.one);
    if (pathDirString[avaIndex] == DirectionEnum.Left || pathDirString[avaIndex] == DirectionEnum.Right) {
      tempObj.GetComponent<TopAndBottomCardScript>().setLefAndRightPoint(cardPoint);
  ...
  private void frontBuhuaBeginPickCard(int avaIndex, int cardPoint) {
  ...
  private void frontIncard(int avaIndex,int cardPoint){
	...
	public void pengCard(int avaIndex,int cardPoint)//其他人碰牌
	{
		...
		SoundCtrl.getInstance ().playSoundByAction ("peng",playerItems[avaIndex].getSex(), pathDirString[avaIndex]);
	...
	private void gangCard(int avaIndex,int cardPoint,int gangType){
		...
		SoundCtrl.getInstance ().playSoundByAction ("gang",playerItems[avaIndex].getSex(), pathDirString[avaIndex]);
		List<GameObject> tempCardList = handerCardList[avaIndex];
		string path="Prefab/PengGangCard/PengGangCard_"+pathDirString[avaIndex];
		Vector3 tempvector3 = new Vector3(0, 0, 0);
		if (gangType != 3) {
