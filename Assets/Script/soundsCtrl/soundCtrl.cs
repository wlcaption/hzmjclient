...
public class SoundCtrl  {
  ...
  private static int language_ = 0; // 0 nanjing, 1 putong
  ...
  public AudioSource selectSound(string dir) {
    switch (dir) {
      case DirectionEnum.Left: return soundL;
      case DirectionEnum.Bottom: return soundB;
      case DirectionEnum.Top: return soundT;
      case DirectionEnum.Right: return soundR;
      default: return soundGame;
    }
  }
  ...
  public void playGameBGM() {
    ...
    if (rand == 1) path = "Sounds/gameBGM2";
    AudioClip temp = (AudioClip)soudHash[path];
    if (temp == null) {
      temp = GameObject.Instantiate(Resources.Load(path)) as AudioClip;
      soudHash.Add(path, temp);
  ...
  public void playSoundUI(bool main = false) {
    string path = "";
    if (main) path = "Sounds/other/click01";
    else path = "Sounds/other/click02";
    AudioClip temp = (AudioClip)soudHash[path];
    if (temp == null) {
      temp = GameObject.Instantiate(Resources.Load(path)) as AudioClip;
      soudHash.Add(path, temp);
