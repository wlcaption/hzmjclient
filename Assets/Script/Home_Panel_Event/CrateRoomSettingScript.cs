  ...
  private void onValueChanged(bool value) {
    ...
    for (int i = 0; i < nanjingRoundRule.Count; i++) {
      if (nanjingRoundRule[i].isOn) {
        ...
        if (i == 0) {
          if (panelYuanzi.activeSelf == false) panelYuanzi.SetActive(true);
        } else if (i == 1) {
          if (panelChangkaitou.activeSelf == false) panelChangkaitou.SetActive(true);
        }
        roundtype = i;
      } else {
        nanjingRoundRuleText[i].color = noselectTextColor;
        if (i == 0) {
          if (panelYuanzi.activeSelf) panelYuanzi.SetActive(false);
        } else if (i == 1) {
          if (panelChangkaitou.activeSelf) panelChangkaitou.SetActive(false);
        }
      }
    }
