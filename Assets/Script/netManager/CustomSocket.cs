...
public class CustomSocket {
  ...
  public void Connect() {
    ...
    try {
      IPAddress[] address = Dns.GetHostAddresses(APIS.SocketUrl);
      tcpclient = new TcpClient(address[0].AddressFamily);
      //防止延迟,即时发送!
      tcpclient.NoDelay = true;
      tcpclient.BeginConnect(address[0], APIS.SocketPort, new AsyncCallback(ConnectCallback), tcpclient);
    } catch (Exception ex) {
      //设置标志,连接服务端失败!
  ...
  public void sendMsg(ClientRequest client) {
    Debug.LogFormat("send msg, head code: {0:X}, message: {1}.", client.headCode, client.messageContent);
