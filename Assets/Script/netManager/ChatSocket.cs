...
public class ChatSocket{
	...
	public void Connect()
	{
		...
		try
		{
      IPAddress[] address = Dns.GetHostAddresses(APIS.ChatSocketUrl);
      tcpclient = new TcpClient(address[0].AddressFamily);
      //防止延迟,即时发送!
      tcpclient.NoDelay = true;
      tcpclient.BeginConnect(address[0], 9502, new AsyncCallback(ConnectCallback), tcpclient);
    } catch (Exception ex)
		{
			//设置标志,连接服务端失败!
			Connect();
	...
	private void readBuffer(BinaryReader buffers){
		...
		if(flag == 1){
			...
			Debug.Log("chat.headCode = "+response.headCode+"  sound.lenght =   "+soundLen);
